...
window.addEventListener('DOMContentLoaded' , async(e)=>{
    e.preventDefault()
    try {
        let response = await axios.get(`http://${process.env.SERVER_IP}:3000/expense/getAllDownloadUrl` , {headers: {Authorization: token}})
...
document.getElementById('download').onclick = async function(e){
    e.preventDefault()
    try {
        let response = await axios.get(`http://${process.env.SERVER_IP}:3000/expense/download` , {headers: {Authorization: token}})
        if(response.status === 200){
