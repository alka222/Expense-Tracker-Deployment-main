...
window.addEventListener("DOMContentLoaded",()=>{
    ...
    /*
    axios.get('http://${process.env.SERVER_IP}:3000/expenses',{headers:{"Authorization":token}})
    .then((Response)=>{
  ...
  axios.post(`http://${process.env.SERVER_IP}:3000/expense/addexpense`,myobj,{headers:{"Authorization":token}})
  .then((Response)=>{
  console.log(Response)
  ...
  parentNode.innerHTML= parentNode.innerHTML+childHTML;
    ...
    function deleteUser(userId){
      const token = localStorage.getItem('token')
      axios.delete(`http://${process.env.SERVER_IP}:3000/delete/${userId}`,{headers:{"Authorization":token}})
      .then(()=>{
  ...
  removeExpenseFromScreen(userId)
    ...
    document.getElementById('rzp-button1').onclick = async function(e){
        ...
        const token = localStorage.getItem('token')
        try {
            const response = await axios.post(`http://${process.env.SERVER_IP}:3000/payment/premiummembership`, x, {headers : {'Authorization': token}})
  ...
  function showLeaderBoard(){
    ...
    inputElement.onclick= async()=>{
      const token = localStorage.getItem('token')
      const userLeaderBoardArray = await axios.get(`http://${process.env.SERVER_IP}:3000/premium/showLeaderBoard`,{headers : {'Authorization': token}})
  ...
  function showPremiumuserMessage(){
    ...
    function checkout(order){
        ...
        var options = {
            ...
            "handler": function (response) {
                ...
                axios.post(`http://${process.env.SERVER_IP}:3000/payment/updatestatus`, response,
    ...
    function download(){
      const token = localStorage.getItem('token')
      axios.get(`http://${process.env.SERVER_IP}:3000/expense/download`,{headers : {'Authorization': token}})
      .then((response)=>{
    ...
    async function getLoadExpenses(page , itemsPerPage){
      const token = localStorage.getItem('token')
      try {
          let response = await axios.post(`http://${process.env.SERVER_IP}:3000/expense/${page}` ,{itemsPerPage:itemsPerPage}  , {headers:{"Authorization" : token}})
